...
lprec <- list(theta=list(prior="pc.prec", param=c(5, 0.01)))

### the likelihood part
lhood <- like(formula = y ~ .,
              family="gaussian",
              control.family = list(
                  hyper = lprec),
              data=ldata)
...
print(summary(component_list(M0f, lhoods=list(lhood), verbose=FALSE)))
...
### define the spacetime model
    ...
    stmodel <- stModel.define(
        ...
        control.priors=list(
            ...
            psigma=c(5, 0.01)), 
    constr = TRUE) ## add an constraint: integrated-to-zero
...
### set overall INLA options
    ...
    fit <- bru(
        ...
        options = list(
            verbose=TRUE,
            num.threads="12:4", ### with 48 threads
            control.compute=ccomp,
            control.mode=list(theta=theta.ini, restart=TRUE),
            control.inla = list(int.strategy = "eb")))
