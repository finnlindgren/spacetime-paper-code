...
cat(paste0("Average node distance (km): ",
           paste0(
             "v = ",
             round(
               c(
...
for(i in 1:5) {
    ...
    results[[i]]$mpred <- readRDS(file.path(
        data.dir,
        paste0("tavg_m", i-1, "_mpred_u.rds"))) ## NEW: tavg_m[0-4]_mpred_u.rds
}
...
rbind("mean(sigma)"=sapply(results[2:5], function(r)
...
### Histograms
    ...
    for(imodel in 2:5) {
        hist(results[[imodel]]$summary.random$spacetime$mean, 100, xlab="", ylab="",
...
BB0 <- cbind(
    ...
    Blat0[iibb1, 4] * Btime0[iibb2, ]) ### north
...
world <- worldMap()
wx0y0 <- list(x=seq(bb[1,1]+40, bb[1,2], 80),
              y=seq(bb[2,1]+20, bb[2,2], 80))
sapply(wx0y0, length)
wxy0 <- SpatialPoints(as.matrix(
...
### world box
wbox0 <- cbind(
    long=c(seq(-1, 1, length=201), rep(1, 101),
           seq(1, -1, length=201), rep(-1, 101))*179.99999,
    lat=c(rep(1, 201), seq(1,-1, length=101),
          rep(-1, 201), seq(-1, 1, length=101))*89.99999)
...
### which pixels fall in the whole Earth or only over land
    ...
    cat("Pixels in", c("whole Earth", "land")[el], ":",
        length(ii.wxy0), "(",
...
##        ab.st <- max(abs(sapply(v.grid, range)))
  ##      ab.st <- max(40, ab.st)
        ...
        figfl <- paste0("slow_st", c("Earth", "Land")[el],
            ...
            for(i in 1:n.v) {
                ...
                cat("E(v|y,M", LETTERS[i], ") range: ",
                    paste(format(range(wzz, na.rm=TRUE), digits = 4),
                          collapse=" to "), "\n", sep="")
                wzz_raster <- rast(
                  data.frame(x = rep(wx0y0$x, times = length(wx0y0$y)) * 1000,
...
### project selected time to the grid
        u.grid <- lapply(results[-1], function(r)
            inla.mesh.project(
                pgridl, matrix(r$summary.ran$spacetime$mean,
                               gmesh$n)[, jj1]))
        (n.u <- length(u.grid))
...
#        ab.st <- max(abs(sapply(u.grid, range)))
 #       ab.st <- max(40, ab.st)
        ...
        figfl <- paste0("st", c("Earth", "Land")[el],
                        "4m", gmesh$n, "_", tmesh$n,
                        "t", paste0(jj1, collapse="_"), ".png")
        st4mfl <- file.path(fig.dir, figfl)
        cat("Figure file:", st4mfl, "\n")
