...
saveRDS(object = stations.longlat,
        file = stations.file)
cat("Saved data on", nrow(stations.longlat),
    "selected stations to", stations.file, "\n")

### data information resume
c(nstations <- dim(wtavg)[1], 
  ntimes <- dim(wtavg)[2])
...
### save
save(list = c("Blat.mesh", "time.basis"), 
     file = Bbasis.file)
