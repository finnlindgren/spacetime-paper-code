...
handle_packages <- function(needed) {
  ...
  version_ok <- vapply(names(needed),
                       function(x) {
  ...
  special_installation <-
    c("INLA" = 'install.packages("INLA", repos = c(options("repos", INLA="https://inla.r-inla-download.org/R/testing")))',
  ...
  for (pkg in names(needed)[!version_ok]) {
    ...
    if (!installed[pkg]) {
      warning(paste0("The ", pkg, " package is not installed. Install with\n",
                     "  ", inst),
              call. = FALSE)
    } else {
        warning(
